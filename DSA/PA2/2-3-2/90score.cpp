...
int get_index_by_scanf()
{
    ...
    {
        scanf("%d", &x);
        if (x < nodes[index].sons.get_size())
...
void recursive_set_height(int son_index /*already set*/, int father_index /*not set yet*/)
{
    if (delta_height == 0)
        return;
    if (father_index == 0) // ensure father_index isn't 0
        return;
    if (delta_height < 0) // cut
    {
        ...
        if (father.father)
            recursive_set_height(father_index, father.father);
    }
    else // link
    {
        TreeNode &son = nodes[son_index];
...
void cut(int son_index)
{
    ...
    if (delta_height)                                      // if height changed
        recursive_set_height(father_index, father.father); // reset all heights, start from grandpa
