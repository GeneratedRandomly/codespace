...
Board UCT::nowOpponentBoard, UCT::nowMyBoard;
...
int UCT::calculateProfit(const Board &opponentBoard, const Board &myBoard, const int rest, const int player)
{
...
const UCT::MoveResult UCT::getOptimalMove(Board &thatBoard, int opponentLastX, int opponentLastY)
{
...
double UCT::defaultPolicy(TreeNode *nowNode)
{
	...
	{
		Board *boards[] = {&nowMyBoard, &nowOpponentBoard};
...
std::pair<int, int> UCT::UCTSearch(int const *const *boardStart, const int *topStart)
{
	...
	auto originalMyBoard = Board{M, N, myBoard};
	auto originalOpponentBoard = Board{M, N, opponentBoard};
