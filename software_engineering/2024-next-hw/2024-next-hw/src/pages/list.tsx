                    ...
                    <ul>
                        {boardList.map(board =>
                            <li key={board.id}>
                                <p>
                                    ID: {board.id}<br></br>
                                    NAME: {board.boardName}<br></br>
                                    Created by: {board.userName}<br></br>
                                    Created at: {board.createdAt}<br></br>
                                </p>
                                <button onClick={() => router.push(`/?id=${board.id}`)}>Play it</button>
                                {
                                    board.userName === userName ?
                                        <button onClick={() => deleteBoard(board.id)}>Delete it</button> :
                                        null
                                }
                                <button onClick={() => router.push(`/list?name=${board.userName}`)}>View this user</button>
                            </li>)}
                    </ul>
                    // Step 5 END
                }</div>
